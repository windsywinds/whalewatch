  ...
  useEffect(() => {
    ...
    const handleStandalonePWA = () => {
      ...
      const handleUpdateRequired = () => {
        console.log('App has not been updated recently, updating...');

        const currentTime = new Date().getTime();
        const timeSinceLastReloadInHours = (currentTime - lastReloadTime) / (1000 * 60 * 60);

        if (timeSinceLastReloadInHours < 24) {
          console.log('Less than 24 hours since the last reload, updating...');
          window.location.reload();
          setForceUpdate(prevState => !prevState);
        } else {
          console.log('More than 24 hours since the last reload, skipping update.');
        }
      };

      document.addEventListener('visibilitychange', handleVisibilityChange);
      window.addEventListener('updateRequired', handleUpdateRequired);

      return () => {
        // Cleanup event listeners on component unmount
        document.removeEventListener('visibilitychange', handleVisibilityChange);
        window.removeEventListener('updateRequired', handleUpdateRequired);
      };
    };
...
const MobileMenu = ({handleComponentChange}) => {
  ...
  const generateMenuItemClasses = (component) =>
  `flex flex-col items-center ${activeComponent === component ? 'bg-blue-500' : 'bg-[#282c34]'}`;
  ...
  return(
    ...
    <div className="w-full h-full bg-[#282c34] grid grid-cols-5 divide-x border-t-2 md:border-t-0 md:border-0 text-[#e8e9ff] text-xs">
      ...
      <button className={generateMenuItemClasses('news')} onClick={() => handleComponentSelect('news')}>
      ...
      <button className={generateMenuItemClasses('submit')} onClick={() => handleComponentSelect('submit')}>
      <div className="h-full flex flex-col items-center justify-center">
      ...
      <button className={generateMenuItemClasses('display')} onClick={() => handleComponentSelect('display')}>
      <div className="h-full flex flex-col items-center justify-center">   
      ...
      <button className={generateMenuItemClasses('resources')} onClick={() => handleComponentSelect('resources')}>
      <div className="h-full flex flex-col items-center justify-center">
